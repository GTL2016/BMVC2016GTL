...
In this context, we tested three different approaches. The first one consists in setting the learning parameters uniformly on the network layers. The second one consists in fine-tuning the learning rate of each layer differently. The third one consists in loading the weights of the convolution layers from the classification training. In order to compare our results with those three approaches, we refer to the following loss function :
